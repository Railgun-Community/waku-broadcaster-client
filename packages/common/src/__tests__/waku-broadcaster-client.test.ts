...
describe('waku-broadcaster-client', () => {
  ...
  it('Should start up the client with a failure catch and retry, find best broadcaster, and add transport subscription', async () => {
    ...
    // Stub connect() (this shoudl be identical to the catch statement in connect())
    const connectStub = sinon
      .stub(WakuBroadcasterWakuCore as any, 'connect')
      .callsFake(async () => {
    ...
    expect(WakuBroadcasterWakuCore.connectFailed).to.be.true;
    ...
    const waku = WakuBroadcasterWakuCore.waku; // must use real waku instance to test subscribe
    ...
    await WakuBroadcasterClient.addTransportSubscription(waku, topic, callback);
