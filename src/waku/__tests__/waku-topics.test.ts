...
describe('waku-topics', () => {
  it('Should get correct content topics for mock chain', () => {
    ...
    expect(contentTopics.transact(chain)).to.equal('/railgun/v2/0/1/transact/json');
    expect(contentTopics.transactResponse(chain)).to.equal('/railgun/v2/0/1/transact-response/json');
  });
});
